...
| 检索策略 (Strategy)              | Faithfulness | Answer Relevancy | Context Recall | Context Precision | 核心结论                                                                   |
|:---------------------------------|:-------------|:-----------------|:---------------|:------------------|:---------------------------------------------------------------------------|
| **基线 (Baseline)**              | **0.7871**   | **0.6463**       | **0.7250**     | 0.5917            | 基础检索性能尚可，但精准率(Precision)是明显短板，召回率(Recall)也有提升空间。    |
| **查询扩展 (Query Expansion)**   | 0.5333 (↓)   | 0.0000 (↓↓)      | 0.0500 (↓↓)    | 0.0000 (↓↓)       | **策略失败**。生成的子问题质量不高或跑偏，引入大量噪音，严重污染了上下文，导致系统几乎瘫痪。 |
| **手写HyDE**                     | **0.8090** (↑)   | 0.5834 (↓)       | 0.6667 (↓)     | 0.5625 (↓)        | 忠实度略有提升，但未能改善核心的检索指标，说明生成的假设性文档与真实文档仍有偏差。     |
| **LangChain HyDE**               | 0.7583 (↓)   | 0.5965 (↓)       | **0.7500** (↑)     | 0.5528 (↓)        | **有效提升了召回率(Recall)**，但牺牲了精准率，说明其生成的文档更“泛化”，能匹配到更多内容。 |
| **混合检索 (Hybrid Search - RRF)** | 0.6750 (↓)   | 0.4555 (↓)       | 0.3667 (↓↓)    | **0.6571** (↑)        | **有效提升了精准率(Precision)**，但严重损害了召回率，说明实现中可能存在问题。       |
